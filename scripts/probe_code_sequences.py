...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train/test a linear probe on temporally ordered action-code sequences."
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
    ...
    *,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for sample_indices, label_id in zip(indices, batch["label_id"]):
                one_hot = torch.nn.functional.one_hot(
                    sample_indices,
                    num_classes=cfg.num_codes,
                ).float()
                features.append(one_hot.flatten().numpy())
...
def main() -> None:
    ...
    num_tokens = int(train_dataset.examples[0]["eeg_values"].shape[0])
    frame_encoder = EEGPatchEncoder(
        num_channels=cfg.eeg_num_channels,
        patch_size=cfg.eeg_patch_size,
    ...
    x_train, y_train = extract_sequence_features(
    ...
    x_val, y_val = extract_sequence_features(
        dataset=val_dataset,
        frame_encoder=frame_encoder,
        inverse_dynamics=inverse_dynamics,
        codebook=codebook,
    ...
    probe = LogisticRegression(max_iter=args.max_iter, solver="lbfgs")
